...
export default function PromptWarehouseInterface() {
  ...
  const [selectedCartridge, setSelectedCartridge] =
    useState<BrandCartridge | null>(null);
  ...
  const fetchCartridge = async (storeId: string) => {
    try {
      setIsLoadingCartridge(true);
      setCartridgeError(null);

      const response = await fetch(`/api/clickup/fetch-cartridges/${storeId}`);
      if (!response.ok) {
        throw new Error("Failed to fetch cartridge");
      }

      const data = await response.json();
      setSelectedCartridge(data.cartridge);
    } catch (err) {
      console.error("Error fetching cartridge:", err);
      setCartridgeError("Failed to load cartridge");
  ...
  const handleEdit = () => {
    if (selectedGlobalPrompt) {
      ...
      setEditContent(selectedCartridge.content || "");
    }
    setIsEditing(true);
    setSaveError(null);
    setSaveSuccess(null);
  ...
  const handleSave = async () => {
    try {
      ...
      if (selectedGlobalPrompt) {
      ...
      } else if (selectedStore && selectedCartridge !== null) {
        // Save brand cartridge
        const response = await fetch(
        ...
        if (!response.ok) {
          throw new Error("Failed to save brand cartridge");
        }

        // Update local state
        setSelectedCartridge({ content: editContent });
        setSaveSuccess("Cartridge saved successfully!");
      }
              ...
              <div className="mb-6">
                <div className="flex items-center justify-between">
                  <div className="flex items-center space-x-4">
                    ...
                    <div>
                      <h1 className="text-2xl font-bold text-white">
                        {selectedStore?.name} - Brand Cartridge
                      </h1>
                      <p className="text-sm text-gray-400">
